    ...
    *   generate ior
    ...
    def emitREOP(self, op, in_, frame):
        ...
        if type(in_) is IntType:
            ...
            result.append(self.jvm.emitFCMPL())  # Compare floats, push -1, 0, or 1
            if op == "==":
                result.append(self.jvm.emitIFNE(labelF))  # Jump if not equal (result != 0)
            elif op == "!=":
                result.append(self.jvm.emitIFEQ(labelF))  # Jump if equal (result == 0)
            elif op == ">":
                result.append(self.jvm.emitIFLE(labelF))  # Jump if <= 0 (result <= 0)
            elif op == ">=":
                result.append(self.jvm.emitIFLT(labelF))  # Jump if < 0 (result < 0)
            elif op == "<":
                result.append(self.jvm.emitIFGE(labelF))  # Jump if >= 0 (result >= 0)
            elif op == "<=":
                result.append(self.jvm.emitIFGT(labelF))  # Jump if > 0 (result > 0)
            else:
                raise IllegalOperandException(f"Unsupported operator {op} for FloatType")
        else:
            raise IllegalOperandException(f"Unsupported type {str(in_)}")


        result.append(self.emitPUSHCONST("1", IntType(), frame))
        # frame.pop() nghi ngờ chỗ này không có cần pop
    ...
    # Only compare IntType value with value 0
    def emitREOPSingle(self, op, in_, frame):
        # op: String (e.g., "==", ">", "<=", etc.)
        # in_: Type (expected to be IntType)
        # frame: Frame
        # ..., value -> ..., result (0 or 1)

        result = list()
        labelF = frame.getNewLabel()  # Label for false case
        labelO = frame.getNewLabel()  # Label for end of operation

        frame.pop()  # Pop the single operand
        # Đoạn này từ từ suy nghĩ lại xem sao emitIFNE và emitIFEQ phải đổi ngược vị trí cho nhau trong
        # điều kiện == và !=
        if type(in_) is IntType:
            if op == "==":
                result.append(self.jvm.emitIFNE(labelF))
            elif op == ">":
                result.append(self.jvm.emitIFLE(labelF))  # value <= 0 jumps to false
            elif op == ">=":
                result.append(self.jvm.emitIFLT(labelF))  # value < 0 jumps to false
            elif op == "<":
                result.append(self.jvm.emitIFGE(labelF))  # value >= 0 jumps to false
            elif op == "<=":
                result.append(self.jvm.emitIFGT(labelF))  # value > 0 jumps to false
            elif op == "!=":
        ...
        # True case: push 1 and jump to end
        result.append(self.emitPUSHCONST("1", IntType(), frame))
        result.append(self.emitGOTO(labelO, frame))
