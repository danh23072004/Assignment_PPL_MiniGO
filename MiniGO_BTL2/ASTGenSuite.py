...
"""
        expect = Program([FuncDecl("foo",[],VoidType(),Block([
            Assign(ArrayCell(Id("a"),[BinaryOp("+", IntLiteral(1), IntLiteral(1))]),IntLiteral(1))]))
		])
        self.assertTrue(TestAST.test(input, str(expect), inspect.stack()[0].function))
...
"""
    ...
    def test_114(self):
        ...
        expect = Program([ConstDecl("VoTien", None,
...
"""
        expect = Program([FuncDecl("foo",[],VoidType(),Block([
            ...
            FuncCall("a",[IntLiteral(1),IntLiteral(2)]),
            FuncCall("a",[IntLiteral(1)]),
            MethCall(FieldAccess(FieldAccess(FieldAccess(FieldAccess(Id("b"),"a"),"a"),"a"),"a"),"a",[]),
            MethCall(FieldAccess(Id("b"),"a"),"a",[IntLiteral(1),IntLiteral(2)]),
            MethCall(FieldAccess(Id("b"),"a"),"a",[IntLiteral(1)])]))
		])
        self.assertTrue(TestAST.test(input, str(expect), inspect.stack()[0].function))
    ...
    def test_153(self):
        ...
        """
        expect = Program([
            FuncDecl("fvoid",[],VoidType(),Block([FuncCall("putStringLn",[StringLiteral("\"VoTien\"")])]))
            ,VarDecl("global",None,FuncCall("fint",[]))
            ,FuncDecl("main",[],VoidType(),Block([
                FuncCall("fvoid",[]),
                FuncCall("putFloatLn",[BinaryOp("+", Id("global"), FloatLiteral("2.0"))]),
                Assign(Id("d"),IntLiteral(1)),
                VarDecl("local",None,StringLiteral("\"a\"")),
                FuncCall("putBoolLn",[BinaryOp("<=", Id("local"), StringLiteral("\"b\""))]),
                Assign(Id("local"),BinaryOp("+", Id("local"), StringLiteral("\"c\""))),
                FuncCall("putStringLn",[Id("local")])
            ]))
            ,FuncDecl("fint",[],IntType(),Block([Return(IntLiteral(1))]))
        ])
        self.assertTrue(TestAST.test(input, str(expect), inspect.stack()[0].function))
